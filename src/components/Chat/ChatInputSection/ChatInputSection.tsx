...
export function ChatInputSection({
  ...
  const uploadFile = async (file: File) => {
    try {
      ...
      if (file.size > TWENTY_MB) {
        toast.error("アップロードファイルが大きすぎます。最大20MBです。");
        return;
      }
  ...
  return (
    <>
      {fileDragging && (
        <DragFile>
          <Title theme={theme}>送信するファイルをドロップ</Title>
        </DragFile>
      )}
        ...
        <ReplyContainer theme={theme}>
          <div>
            <ReplyTitle theme={theme}>
              <BsFillReplyFill />
              <p>
                {currentUser?.uid === replyInfo.sender
                  ? " 自分のメッセージを"
                  : ""}
                引用する
              </p>
            ...
            {replyInfo.type === "text" ? (
              ...
              <ReplyText theme={theme}>画像にリプライ</ReplyText>
            ) : replyInfo.type === "file" ? (
              <ReplyText theme={theme}>ファイルにリプライ</ReplyText>
            ) : (
              "メッセージの送信を取り消しました"
            )}
          </div>
