...
export function Admin({ conversation, theme }: AdminProps) {
  ...
  const handleRemoveAdminPosition = (uid: string, targetUserName: string) => {
    updateDoc(
    ...
    toast.success(`${targetUserName}の管理者権限を削除`);
  };
        ...
        .map((user) => (
          <MemberWrapper key={user.uid}>
            ...
            user.uid !== currentUser?.uid ? (
              <RemooveAdminButton
                onClick={() =>
                  handleRemoveAdminPosition(user.uid, user.displayName)
                }>
                管理者権限を削除
              </RemooveAdminButton>
            ) : (
              <AdminBadge>管理者</AdminBadge>
            )}
          </MemberWrapper>
        ))}
    </MemberContainer>
